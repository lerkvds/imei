...
echo 'NetflixCredit - Code by JamvanHax0r'. PHP_EOL;
echo 'Support: MY TH PH'. PHP_EOL;
echo '-----------------'. PHP_EOL;
sleep(1);
echo '> Masukkan UID (bebas): ';
...
sleep(1);
echo '> Masukkan awalan IMEI ('.$__imei.'): ';
$input = fopen("php://stdin","r");
$answer = trim(fgets($input));
if($answer!=='') {
	$__imei=$answer;
...
sleep(1);
echo '> Masukkan panjang IMEI ('.$__length.'): ';
$input = fopen("php://stdin","r");
$answer = trim(fgets($input));
if($answer!=='') {
	$__length=(int)$answer;
